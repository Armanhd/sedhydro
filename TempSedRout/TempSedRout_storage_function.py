...
def TempSedRout_storage (river_gdf, sediment_size, config_file, h, q, Q, 
                 width, SSC_hru_frac, 
                 ...
                 river_storage, storage_data_type,
                 storage_coef_fl,
                 storage_coef_fh,
                 storage_coef_fw,
                 storage_coef_fa):
    ...
    storage_links = set(river_storage['LINKNO'])

    # Create the Boolean column
    river_gdf['storage_reach'] = river_gdf.index.isin(storage_links)
    ...
    E_dict = {rid: {k: np.zeros_like(arr) for k, arr in frac.items()}
              for rid, frac in SSC_river_frac.items()}
    # generate phi_i_dict with all ones (dimensionless removal coefficient)
    phi_i_dict = {
        rid: {k: np.zeros_like(arr) for k, arr in frac.items()}
        for rid, frac in SSC_river_frac.items()}
    ...
    def get_input_array(data, reach_id=None, eps=1e-12):
        """
        Convert DataFrame, Series, NumPy array, list, or scalar to numeric array.
        """
        if data is None:
            return None
    
        if isinstance(data, pd.DataFrame):
            arr = data[reach_id].to_numpy(dtype=float)
    ...
    def storage_removal_phi(
        ...
        *,
        ...
        storage_data_type = storage_data_type.lower()
    
        h_ts = get_input_array(h, reach_id, eps)
        u_ts = get_input_array(u, reach_id, eps)
        Q_ts = get_input_array(Q, reach_id, eps)
    
        if storage_data_type == "length":
            exponent = -(ws_i * storage_coef_fl * length) / (
                storage_coef_fh * h_ts * u_ts
            )
    
        elif storage_data_type == "width":
            exponent = -(ws_i * storage_coef_fw * width) / Q_ts
    
        elif storage_data_type == "area":
            exponent = -(ws_i * storage_coef_fa * area) / Q_ts
    
        else:
            raise ValueError("storage_data_type must be 'length', 'width', or 'area'")
    
        phi_i = 1.0 - np.exp(exponent)
        phi_i = np.clip(phi_i, 0.0, 1.0)
    
        return phi_i

    #%% calculate velocity for storage length option
    # u = q / h, where q is unit flow [m2/s] and h is depth [m]
    uinterp = {}

    for rid in qinterp:
        uinterp[rid] = qinterp[rid] / np.maximum(hinterp[rid], 1e-12)
    #%% calculate phi_i
    #!!!
    
                
    for reach_id in river_gdf.index:   # reach_id is ALWAYS a LINKNO in river_gdf.index
    
        if river_gdf.loc[ reach_id, 'storage_reach']:
            print(f"storage for reach {reach_id}")
            for k in range (0, len(di)):   # loop for each fraction
                if storage_data_type == "length":

                    phi_i_dict[reach_id][k] = storage_removal_phi(
                        storage_data_type="length",
                        ws_i=falveli[k],
                        reach_id=reach_id,
                        length=river_storage.loc[
                            river_storage["LINKNO"] == reach_id,
                            "Length_m"
                        ].iloc[0],
                        h=hinterp[reach_id],
                        u=uinterp[reach_id]
                    )
                if storage_data_type=="width":
            
                    phi_i_dict[reach_id][k]=storage_removal_phi(
                                                                storage_data_type="width",
                                                                ws_i=falveli[k],
                                                                reach_id=reach_id,
                                                                width=river_storage.loc[river_storage["LINKNO"] == reach_id, "Width_m"].iloc[0],
                                                                Q=Qinterp[reach_id]
                                                            )            
                if storage_data_type=="area":
            
                    phi_i_dict[reach_id][k]=storage_removal_phi(
                                                                storage_data_type="area",
                                                                ws_i=falveli[k],
                                                                reach_id=reach_id,
                                                                area=river_storage.loc[river_storage["LINKNO"] == reach_id, "Areas_m2"].iloc[0],
                                                                Q=Qinterp[reach_id]
                                                            )
        ...
        if len(us) == 2:
            ...
            # i=0# one fraction
                ...
                for j in range (1, num_time): # loop at each time step
                    ...
                    # impact of storage on C
                    Cleft_array=(1-phi_i_dict[n_us1][k][:,j])*Cleft_array
                ...
                for j in range (1, num_time): # loop at each time step
                    ...
                    # impact of storage on C
                    Cleft_array=(1-phi_i_dict[n_us2][k][:,j])*Cleft_array
                    # change negative C to zero (-C=0)
                    Cleft_array[Cleft_array<0]=0
        ...
        # 1-tributary
            ...
            # i=0# one fraction
                ...
                for j in range (1, num_time): # loop at each time step
                    ...
                    # impact of storage on C
                    Cleft_array=(1-phi_i_dict[n_us][k][:,j])*Cleft_array
